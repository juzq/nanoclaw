...
// Import NanoClaw types and functions
...
function createIpcMessage(data: RegisterGroupData): void {
...
Example usage:
node add-new-group.ts --jid "feishu:newgroup123" --name "New Group" --folder "newgroup" --trigger "@Andy"
...
if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
  console.log(`
Usage: node add-new-group.ts [options]
...
Examples:
  node add-new-group.ts --jid "feishu:mygroup123" --name "My Team" --folder "myteam"
  node add-new-group.ts --jid "feishu:project-x" --name "Project X" --folder "projectx" --trigger "@Assistant"
  `);
  process.exit(0);
}
